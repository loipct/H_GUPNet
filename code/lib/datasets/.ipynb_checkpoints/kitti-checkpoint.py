...
class KITTI(data.Dataset):
    def __init__(self, root_dir, split, cfg):
        ...
        # data split loading
        assert split in ['train', 'val', 'trainval', 'test','train_temp']
        self.split = split
    ...
    def __getitem__(self, item):
        ...
        if self.split!='test':
            ...
            center_3dto2d = np.zeros((self.max_objs, 2), dtype=np.float32)
            rys=np.zeros((self.max_objs, 1), dtype=np.float32)
            Qgt=np.zeros((self.max_objs, 5,3), dtype=np.float32)
            Pgt=np.zeros((self.max_objs, 5,2), dtype=np.float32)
            ...
            for i in range(object_num):
                ...
                # center_3d_imgs_pro[i]=  center_3d
                ...
                rys[i]=objects[i].ry
                pos[i]=objects[i].pos 
                center_3dto2d[i]= center_heatmap #calib.rect_to_img(objects[i].pos.reshape(-1,3))[0] 
                Qgt[i]= np.concatenate([objects[i].pos.reshape(-1,3),objects[i].generate_corners3d()[:4,:]],0) 
                 
            Pgt[:object_num]= calib.corners3d_to_img_boxes(Qgt[:object_num])[1]
#             print(object_num)
            targets = {'depth': depth,
                        'size_2d': size_2d,
                        'heatmap': heatmap,
                        ...
                        'cls_ids': cls_ids,
                        'mask_2d': mask_2d,
                        'Qgt'    : np.concatenate([pos.reshape(-1,1,3), Qgt[:,:4,:]],1),
                        'Pgt'    : np.concatenate([calib.rect_to_img(pos)[0].reshape(-1,1,2), Pgt[:,:4,:]],1),
                        'center_3dto2d':center_3dto2d,
                        'ry':rys
                   } 
        else:
            targets = {}
        ...
        info = {'img_id': index,
                ...
                'trans_inv': trans_inv,
                'crop_status': random_crop_flag,
                'flip_status': random_flip_flag}   
        return inputs, calib.P2, coord_range, targets, info   #calib.P2
    
    # def get_Qgt(self,object):
    #     point = np.concatenate([object.pos.reshape(1,3), object.generate_corners3d()[:4,:]],0)
    #         #print(point.shape)
    #     return point #5x3
