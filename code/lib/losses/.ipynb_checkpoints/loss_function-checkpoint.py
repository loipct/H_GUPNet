...
class Hierarchical_Task_Learning:
    def __init__(self,epoch0_loss,stat_epoch_nums=5,past_losses=None,init_diff =None):
        ...
        self.past_losses=past_losses if past_losses else []
        self.init_diff = init_diff
        self.loss_graph = {'seg_loss':[],
                           ...
                           'homography_loss':['depth_loss','offset3d_loss']}                          
    def compute_weight(self,current_loss,epoch):
        ...
        for term in self.loss_graph:
            if term == 'homography_loss':
                loss_weights[term] = torch.tensor(0.0).to(current_loss['depth_loss'].device) 
            else:
...
def compute_heading_loss(input, mask, target_cls, target_reg):
    ...
    # cls_onehot = torch.zeros(target_cls.shape[0], 12).scatter_(dim=1, index=target_cls.view(-1, 1), value=1)
